##############################################################################
# Prod environment — es-data-hot terragrunt.hcl
# 3 r6i.2xlarge data nodes across 3 AZs
##############################################################################
...
dependency "s3"  { config_path = "../s3-buckets"; mock_outputs = { kms_key_arn = "arn:aws:kms:us-east-1:333:key/mock" } }
...
inputs = {
  ...
  data_volume_iops             = 9000
  data_volume_throughput_mbps  = 700
